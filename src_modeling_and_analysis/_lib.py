...
def notnoise_subset(df):
    notnoise_cats = ['del', 'del_notatcut', 'ins', 'ins_notatcut', 'combination_indel', 'forgiven_indel',
                     'forgiven_combination_indel', 'wildtype', 'del_notcrispr', 'ins_notcrispr']
    criteria = df['Type'].isin(notnoise_cats)
    return df[criteria]


def crispr_subset(df):
    crispr_cats = ["DELETION", "INSERTION"]
    criteria = df['Type'].isin(crispr_cats)
    return df[criteria]
...
def get_sequence_cutsite(df):
    return set(df['exp']).pop(), set(df['cutSite']).pop()
...
def merge_crispr_events(df1, df2, nm1, nm2):
    join_cols = ['Category', 'Genotype Position', 'Indel with Mismatches', 'Ins Fivehomopolymer', 'Ins Template Length',
                 'Ins Template Orientation', 'Ins mh2', 'Ins p2', 'Inserted Bases', 'Length', 'Microhomology-Based']
